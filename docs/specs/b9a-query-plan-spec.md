...
| `governance_policy` | text_search(multi_query) → entity_lookup → relationship_traverse(1-hop) | standard | top_k=12, el=8, rel=30 | B9c baseline |
...
- **Used by:** governance_policy (B9c baseline). Removed from relationship_path in B9c (over-retrieval).
