#!/usr/bin/env python3
"""
MCP Server for BGE Semantic Search with Improved Initialization
...
async def main():
    ...
    # Initialize server before starting stdio
    logger.info("Pre-initializing BGE server resources...")
        ...
        async with stdio_server() as (read_stream, write_stream):
            ...
            async with ServerSession(read_stream, write_stream, init_options) as session:
                ...
                async def call_tool(name: str, arguments: dict) -> List[TextContent]:
                    try:
                        if name == "bge_search":
                            result = await server.search_embeddings(
