...
**Status:** REGISTERED, AMENDED r2 — pending prereg-skeptic PROCEED. No
implementation run before that verdict. **Registered:** 2026-08-26 ·
...
- `koi-membrane-wt` HEAD (the branch code actually executing);
...
- **clean-slate precondition (F1):** `canon_dependency_impacts` and
  `canon_review_cases` are empty and `GET /tasks/?source_type=canon-review`
  returns `[]` on BOTH nodes;
...
| B2 | **Exactly 1** open `canon_review_case`, keyed to `…:C5` specifically | case-per-impact (→2); coalescer keyed by a constant (would also case the C1 control) | C1 negative control gives coalescing a second assertion NOT to merge with |
...
| B5 | Task visible on NUC via `GET /tasks/?source_type=canon-review` within **30 min** of creation, receipt per §7 | D7-class silent no-op emitter; federation defect | preflight (§1a) was green, so ambient-transport failures void rather than fail |
...
| B8 | **Negative control:** the C1→chain-head dependency yields **0** impacts, **0** cases, **0** tasks, and no C1 line in the brief | fire-on-everything watcher; constant-keyed coalescer | chain head verified to have zero supersessions above it |
...
- **No development executions against `personal_koi`** (§2 containment).
