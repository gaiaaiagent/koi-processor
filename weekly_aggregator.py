...
class WeeklyAggregator:
    ...
    def generate_brief(self, digest: WeeklyDigest) -> str:
        ...
        for i, story in enumerate(digest.top_stories[:5], 1):
            ...
            # Content summary
            content = story.content[:300].replace('\n', ' ')
        ...
        target = self.config["content"]["brief_word_count"]
        
        if word_count < 800:
            # Add more detail
            lines.insert(-2, "\n## Additional Context\n")
            lines.insert(-2, "The Regen Network continues to advance its mission of ecological regeneration through coordinated action across technology, governance, and community engagement. ")
            lines.insert(-2, "This week's developments reflect the growing momentum in regenerative finance and ecological data infrastructure.\n")
            brief = ''.join(lines)
        elif word_count > 1200:
            # Truncate if needed
            brief = ' '.join(brief.split()[:1200]) + "...\n\n*[Content truncated for brevity]*"
